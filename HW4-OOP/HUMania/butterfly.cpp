...
void Butterfly::draw()
{
    ...
    if (move_down == true)
    { // move down variable is true initially
    ...
    else if (move_down == false)
    {
        ...
        if (moverRect.y <= 0)
        { // if it hits the top, move_down gets true and the process repeats
            move_down = true;
        }
    }

    if (moverRect.x > 1000)
    { // if the object reaches the end of the right screen, the x pixels are set to -30 to have a continuous motion
        moverRect.x = -30;
    }
...
{
    if (state == 0)
    { // the initial state of the butterfly
        srcRect = {256, 24, 174, 134};
        state++; // the state changes to the next state
    }

    else if (state == 1)
    { // the second state of the butterfly
        srcRect = {257, 182, 192, 214};
        state++; // the state changes to the next state
    }

    else if (state == 2)
    { // the final state of the butterfly
